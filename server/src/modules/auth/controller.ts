...
function tokensFor(userId: string, tokenVersion = 0) {
  return { access: signAccess(userId, tokenVersion), refresh: signRefresh(userId, tokenVersion) };
}
...
export async function register(req: Request, res: Response) {
  ...
  const tokens = tokensFor(String(user._id), user.tokenVersion || 0);
...
export async function refresh(req: Request, res: Response) {
  ...
  // Reject refresh tokens minted before the latest revocation.
  if ((user.tokenVersion || 0) !== (payload.tv || 0)) throw new ApiError(401, "Session has been revoked.");

  res.json({ access: signAccess(String(user._id), user.tokenVersion || 0) });
}
