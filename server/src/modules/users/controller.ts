...
export async function update(req: Request, res: Response) {
  ...
  if (body.password) {
    ...
    // A new password invalidates any tokens issued under the old one.
    user.tokenVersion = (user.tokenVersion || 0) + 1;
  }
  // Blocking an account also revokes its live sessions immediately.
  if (body.is_active === false) user.tokenVersion = (user.tokenVersion || 0) + 1;
