...
import { TicketStatus, Priority, TERMINAL_STATUS, SLA_POLICY } from "../../constants.js";
...
/** GET /api/tickets/escalations — open tickets that are high-priority or breaching SLA. */
export async function escalations(_req: Request, res: Response) {
  const open = await Ticket.find({ status: { $nin: TERMINAL_STATUS } })
    .sort({ priority: -1, createdAt: 1 })
    .populate("createdBy", "username email isStaff isSuperuser")
    .populate("assignedTo", "username email isStaff isSuperuser");

  const results = open
    .map((t) => ({ t, sla: slaStatus(t) }))
    .filter(({ t, sla }) =>
      t.priority === "critical" || t.priority === "high" || sla.first_response.breached || sla.resolution.breached
    )
    .map(({ t, sla }) => serializeTicket(t, { sla }));

  const critical = results.filter((r) => r.priority === "critical").length;
  const breached = results.filter((r) => r.sla?.first_response.breached || r.sla?.resolution.breached).length;

  res.json({ count: results.length, critical, breached, results });
}

/** GET /api/tickets/sla-report — SLA policy + live compliance across open tickets. */
export async function slaReport(_req: Request, res: Response) {
  const open = await Ticket.find({ status: { $nin: TERMINAL_STATUS } })
    .populate("createdBy", "username email isStaff isSuperuser")
    .populate("assignedTo", "username email isStaff isSuperuser");
  ...
  for (const t of open) {
    ...
    const fr = sla.first_response.breached;
    const rz = sla.resolution.breached;
    if (fr) frBreached++;
    if (rz) resBreached++;
    if (fr || rz) breaching.push(serializeTicket(t, { sla }));
  }
  ...
  res.json({
    ...
    resolution_breaches: resBreached,
    compliance_percent: open.length
      ? Math.round(((open.length - Math.max(frBreached, resBreached)) / open.length) * 100)
      : 100,
    breaching,
  });
}
