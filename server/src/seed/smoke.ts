...
async function main() {
  ...
  // ---- Dedicated reports ----
  const esc = await call("GET", "/api/tickets/escalations", undefined, adminToken);
  check("escalations endpoint", esc.status === 200 && typeof esc.data.count === "number" && Array.isArray(esc.data.results), esc.data);
  const slaRep = await call("GET", "/api/tickets/sla-report", undefined, adminToken);
  check("sla-report endpoint", slaRep.status === 200 && typeof slaRep.data.compliance_percent === "number" && !!slaRep.data.policy, slaRep.data);
  const teamStats = await call("GET", "/api/teams/stats", undefined, adminToken);
  check("teams stats endpoint", teamStats.status === 200 && Array.isArray(teamStats.data.teams) && Array.isArray(teamStats.data.agents), teamStats.data);
  const teamForbidden = await call("GET", "/api/teams/stats", undefined, custToken);
  check("teams stats staff-only", teamForbidden.status === 403);

  // ---- Validation (zod) ----
  const badTicket = await call("POST", "/api/tickets", { description: "no subject" }, custToken);
  check("validation rejects ticket without subject", badTicket.status === 400, badTicket.data);
  const badPriority = await call("POST", "/api/tickets", { subject: "x", priority: "urgent" }, custToken);
  check("validation rejects bad priority enum", badPriority.status === 400, badPriority.data);
  const badRegister = await call("POST", "/api/register", { username: "z", password: "123" });
  check("validation rejects short password", badRegister.status === 400, badRegister.data);

  // ---- Token revocation on block ----
  const victim = await call("POST", "/api/register", { username: "victim", password: "victim123" });
  const victimToken = victim.data.access;
  const beforeBlock = await call("GET", "/api/me", undefined, victimToken);
  check("token valid before block", beforeBlock.status === 200);
  await call("PATCH", `/api/users/${victim.data.user.id}`, { is_active: false }, adminToken);
  const afterBlock = await call("GET", "/api/me", undefined, victimToken);
  check("token revoked after block", afterBlock.status === 401, afterBlock.status);
