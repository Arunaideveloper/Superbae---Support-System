...
function SelectTrigger({ className, children, ...props }: React.ComponentProps<typeof SelectPrimitive.Trigger>) {
  return (
    <SelectPrimitive.Trigger
      ...
      className={cn(
        "flex h-11 w-full items-center justify-between gap-2 rounded-xl border border-input bg-card px-3.5 py-2 text-sm text-foreground outline-none transition-[color,box-shadow] cursor-pointer",
        "focus-visible:border-[#d8c6f7] focus-visible:ring-[3px] focus-visible:ring-[#d8c6f7]/35 disabled:cursor-not-allowed disabled:opacity-50 data-[placeholder]:text-[#b9b1b7]",
  ...
  return (
    <SelectPrimitive.Portal>
      <SelectPrimitive.Content
        ...
        className={cn(
          "relative z-50 max-h-[--radix-select-content-available-height] min-w-32 overflow-hidden rounded-xl border bg-popover text-popover-foreground shadow-lg",
...
function SelectItem({ className, children, ...props }: React.ComponentProps<typeof SelectPrimitive.Item>) {
  return (
    <SelectPrimitive.Item
      ...
      className={cn(
        "relative flex w-full cursor-pointer items-center gap-2 rounded-lg py-2 pr-8 pl-3 text-sm outline-none select-none",
